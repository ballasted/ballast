"""
BALLAST manatee collection — deterministic SVG from token id.

Depth is the primary axis and comes straight from the id: #1 sits just under
the surface, #1000 rests on the seabed. Everything else is environment drawn
from a seeded hash, so no two look the same but nothing outranks anything.

Nothing here is rarer than anything else. Just deeper.
"""

import os, math, hashlib, cairosvg

OUT = "/mnt/user-data/outputs/ballast-social/manatee-collection"
os.makedirs(OUT, exist_ok=True)

SUPPLY = 1000
S = 1000                       # square, NFT-standard
...
def water_at(t):
    """Ground colour by depth t in [0,1]: shallow green-black to deep near-black."""
    top = (10, 26, 14)
    bot = (3, 6, 4)
...
def build(token_id):
    t = (token_id - 1) / (SUPPLY - 1)          # 0 at the surface, 1 at the seabed

    ground = water_at(t)
    b = f'<rect width="{S}" height="{S}" fill="{ground}"/>'

    # ── the surface, receding upward as depth increases
    #    id 1: waterline low in frame. id 1000: waterline far above, off-canvas.
    WL = lerp(300, -260, t)
    if WL > -60:
        amp = lerp(9, 3, t)
        pts = []
    ...
    # ── light shafts, only near the surface
    shafts = 0
    if t < 0.55:
    ...
    # ── suspended particles, denser with depth
    ...
    # ── seabed, only in the deepest third
    bed = t > 0.68
    if bed:
        k = (t - 0.68) / 0.32
...
if __name__ == "__main__":
    ...
    for i in sample:
        svg, tr = build(i)
        with open(f"{OUT}/{i:04d}.svg", "w") as f:
            f.write(svg)
        cairosvg.svg2png(bytestring=svg.encode(),
                         write_to=f"{OUT}/{i:04d}.png", output_width=1000)
